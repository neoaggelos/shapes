...
EnterName::EnterName(string msg)
{
    ...
    ok = CreateButton("ok", "OK", SDL_RECT(160, 400, 160, 40), 20, callback, &done, SDL_SCANCODE_RETURN);
}
...
EnterName::openDialog()
{
    ...
    SDL_Rect inputRect = SDL_RECT(100, 230, 280, 50);
    SDL_StartTextInput();
    ...
    while (!done) {
        ...
        gSuper->getTextRenderer()->write(18, message, SDL_RECT(0, 80, 480, 0), Center);

        SDL_RenderDrawRect(renderer, &inputRect);
        if (input.length())
            gSuper->getTextRenderer()->write(18, input, SDL_RECT(inputRect.x + 10, inputRect.y, inputRect.w, inputRect.h), Left, Center);
